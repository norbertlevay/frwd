...
static const struct usb_device_id id_table_cyphidcomrs232[] = {
	...
	{ USB_DEVICE(VENDOR_ID_FRWD, PRODUCT_ID_FRWD) },
	{ }						/* Terminating entry */
};
 ...
 *****************************************************************************/
static int dev_is_FRWD(struct usb_device *dev){

	if ((le16_to_cpu(dev->descriptor.idVendor) == VENDOR_ID_FRWD) &&
	    (le16_to_cpu(dev->descriptor.idProduct) == PRODUCT_ID_FRWD))
	    return 1;
	else
	    return 0;
}
...
static int analyze_baud_rate(struct usb_serial_port *port, speed_t new_rate)
{
	 ...
	 */
/* FRWD begin */
	if (!(dev_is_FRWD(port->serial->dev))) {
